...
class MEDBBK9KRelation(Relation):
...
class MEDBBK9KKG(KG):
    ...
    def get_entity_class(self):
        return MEDBBK9KEntity
    
    def get_relation_class(self):
        return MEDBBK9KRelation
        ...
        if is_attr:
            ...
            if load_cache and os.path.exists(cache_file):
                ...
                with open(file, 'r') as f:
                    ...
                    for line in lines:
                        try:
                            hh, aa, vv = line.strip().split("\t")
                            triples.append((hh, aa, vv))
...
class EADataMEDBBK9K(KGs):
    ...
    def build_dataset(self):
        def load_links(file, is_all_pair=False):
            links = bidict()
            with open(file, 'r') as f:
                for line in f:
                    url1, url2 = line.strip().split("\t")
                    if is_all_pair is False:
                        assert "1@@" in url1, "Error in loading links"
                        assert "2@@" in url2, "Error in loading links"
                        url1 = url1.split("@@")[1]
                        url2 = url2.split("@@")[1]
        ...
        all_pairs = load_links(self.config['ent_links'], is_all_pair=True)
        train_pairs = load_links(self.config['train_links'])
        valid_pairs = load_links(self.config['valid_links'])
        test_pairs = load_links(self.config['test_links'])
