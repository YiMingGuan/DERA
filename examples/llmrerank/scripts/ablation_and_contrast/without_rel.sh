...
do
    echo "Running one shot experiment with parameter Seed: 0 ..." > ${LOG_DIR}/${dataset_file}-shot-qwen32b-chat-attr-num-7-seed-0-ablation-without-rel.log
    python -u test.py \
                --seed 0 \
                ...
                --swap_space 4 \
                --experiment only_attr >> ${LOG_DIR}/${dataset_file}-shot-qwen32b-chat-attr-num-7-seed-0-ablation-without-rel.log 2>&1
done
echo "Without Rel Ablation 32B Done!"

for dataset_file in ${DATASET_FILE[@]}
do
    echo "Running one shot experiment with parameter Seed: 0 ..." > ${LOG_DIR}/${dataset_file}-shot-qwen14b-chat-attr-num-7-seed-0-ablation-without-rel.log
    python -u test.py \
                --seed 0 \
                --config_path config/url_attr/${dataset_file}.yaml \
                --trained \
                ...
                --tensor_parallel_size 1 \
                --swap_space 4 \
                --experiment only_attr >> ${LOG_DIR}/${dataset_file}-shot-qwen14b-chat-attr-num-7-seed-0-ablation-without-rel.log 2>&1
done
