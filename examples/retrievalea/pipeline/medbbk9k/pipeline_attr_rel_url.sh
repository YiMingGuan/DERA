#! /bin/bash
set -e
# This script is used to run the pipeline for MEDBBK9K
export CUDA_VISIBLE_DEVICES=0,1

CONFIG_PATH=config/medbbk9k/attr_rel_url.yaml
LOG_FILE=logs/medbbk9k/attr_rel_url.log

if [ ! -d "logs/medbbk9k" ]; then
  mkdir -p logs/medbbk9k
fi 

GPU_NUM=2

echo ">>>>>> Start pipeline for medbbk9k(attr rel url) " > $LOG_FILE
